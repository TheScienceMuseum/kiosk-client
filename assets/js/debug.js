...
(function () {
  ...
  const templates = {
    package_list_row_template: _.template($('#package_list_row_template')
      .html()),
    log_tail_row_template: _.template($('#log_tail_row_template')
      .html()),
  };
  ...
  ipc.on('packages-update', (event, packages, current_package) => {
    ...
    make_package_current_buttons.on('click', (ev) => {
      const packageData = $(ev.currentTarget)
        .closest('tr')
        .data('package')
        .split('@');
